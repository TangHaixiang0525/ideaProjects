...
public class ApiDetectIniEPGURL_Handler extends DefaultHandler
{
  ...
  public void endDocument()
    throws SAXException
  {
    ApiDetectAppInfo.setUserInfo(this.info);
    super.endDocument();
  }
    ...
    throws SAXException
  {
    if (paramString2.equalsIgnoreCase("epg"))
      ApiDetectAppInfo.nns_tag = paramAttributes.getValue("nns_tag");
    if (paramString2.equalsIgnoreCase("n3_a_a"))
      ApiDetectAppInfo.URL_n3_a_i[0] = paramAttributes.getValue("url");
    label875: 
    do
    {
      do
      {
        ...
        {
          ApiDetectAppInfo.URL_n3_a_i[1] = paramAttributes.getValue("url");
          return;
        }
        if (paramString2.equalsIgnoreCase("n3_a_c"))
        {
          ApiDetectAppInfo.URL_n3_a_i[2] = paramAttributes.getValue("url");
          return;
        }
        if (paramString2.equalsIgnoreCase("n3_a_d"))
        {
          ApiDetectAppInfo.URL_n3_a_i[3] = paramAttributes.getValue("url");
          Logger.d(" ---> 初始化      N3_A_D总入口地址获取成功");
          return;
        }
        if (paramString2.equalsIgnoreCase("n3_a_e"))
        {
          ApiDetectAppInfo.URL_n3_a_i[4] = paramAttributes.getValue("url");
          return;
        }
        if (paramString2.equalsIgnoreCase("n3_a_g"))
        {
          ApiDetectAppInfo.URL_n3_a_i[5] = paramAttributes.getValue("url");
          return;
        }
        if (paramString2.equalsIgnoreCase("n3_a_h"))
        {
          ApiDetectAppInfo.URL_n3_a_i[6] = paramAttributes.getValue("url");
          return;
        }
 ...
 * Qualified Name:     com.starcor.hunan.service.apidetect.parser.ApiDetectIniEPGURL_Handler

...
;$(function($){
	...
	//确认操作
	$('.J_confirmurl').on('click', function(){
		var self = $(this),
	...
	//附件预览
	$('.J_attachment_icon').on('mouseover', function(){
	...
	$('.J_attachment_icons').on('mouseover', function(){
		var ftype = $(this).attr('file-type');
		var rel = $(this).attr('file-rel');
		switch(ftype){
			case 'image':
...
/*
        var j = 1;
        $('.J_cate_select').die('change').on('change', function(){
            var _this = $(this),
            _pid = _this.val();
